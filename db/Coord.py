...
class Coord():
    ...
    def __init__(self, mapId, x, y, version, zoneId, phaseMask, debug=False):
        self.pointList = []
        self.zoneList = {}
        ...
        for mapSet in mapBorders:
            ...
            if (mapId == mId) and (x < x1) and (x > x2) and (y < y1) and (y > y2):
                ...
                if 1 < phaseMask < 4294967295:
                    self.zoneList[zone] = (yCoord, xCoord, phaseMask)
                    self.pointList.append((zone, yCoord, xCoord, phaseMask))
                else:
