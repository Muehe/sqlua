from db.flavor.readDB2ItemTemplate import getItemTemplate

def getItemTables(dictCursor, version, flavor):
    print("Getting item related MySQL tables...")

    ret = {}
    ret['item_template'] = getItemTemplate(version)

    print("  SELECT creature_loot_template")
    dictCursor.execute("SELECT Entry AS id, Item AS item, Chance AS ChanceOrQuestChance, GroupId AS groupid, MinCount AS mincountOrRef FROM creature_loot_template")
    ...
    dictCursor.execute("SELECT Entry AS id, Item AS item, Chance AS ChanceOrQuestChance, GroupId AS groupid, MinCount AS mincountOrRef FROM gameobject_loot_template")
    ...
    dictCursor.execute("SELECT Entry AS id, Item AS item, Chance AS ChanceOrQuestChance, GroupId AS groupid, MinCount AS mincountOrRef FROM item_loot_template")
    ...
    dictCursor.execute("SELECT Entry AS id, Item AS item, Chance AS ChanceOrQuestChance, GroupId AS groupid, MinCount AS mincountOrRef FROM reference_loot_template")
    ...
    dictCursor.execute("SELECT entry AS id, LootId, 0 as VendorTemplateId FROM creature_template_difficulty") # PickpocketLootId and SkinningLootId might be good...
    ...
    dictCursor.execute("SELECT ID AS id, RewardChoiceItemID1 AS RewChoiceItemId1, RewardChoiceItemID2 AS RewChoiceItemId2, RewardChoiceItemID3 AS RewChoiceItemId3, RewardChoiceItemID4 AS RewChoiceItemId4, RewardChoiceItemID5 AS RewChoiceItemId5, RewardChoiceItemID6 AS RewChoiceItemId6, RewardItem1 AS RewItemId1, RewardItem2 AS RewItemId2, RewardItem3 AS RewItemId3, RewardItem4 AS RewItemId4 FROM quest_template")
