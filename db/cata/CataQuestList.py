...
class CataQuestList():
    ...
    def cacheQuests(self, cursor, dictCursor):
        ...
        infile = open(f'data/cata/AreaTrigger.dbc.CSV', 'r')
        a = infile.read()
        infile.close()
        b = re.findall("(.*?),(.*?),(.*?),(.*?),(.*?),(.*?),(.*?),(.*?),(.*?),(.*?),?\n", a)
        areaTrigger = []
        ...
        with open(f'data/cata/quests.pkl', 'wb') as f:
        ...
        for quest in self.qList:
            ...
            if hasattr(self.qList[quest], "creatureStart"):
                for creature in self.qList[quest].creatureStart:
                    if ((self.qList[quest].RequiredRaces & self.raceIDs['ALLIANCE']) != 0 or self.qList[quest].RequiredRaces == 0) and (
                            self.qList[quest].RequiredRaces not in (self.raceIDs['ALLIANCE'], self.raceIDs['HORDE'])) and npcs.nList[creature].hostileToA:
                        tempRace = tempRace | self.raceIDs['HORDE']
                    if ((self.qList[quest].RequiredRaces & self.raceIDs['HORDE']) != 0 or self.qList[quest].RequiredRaces == 0) and (
                            self.qList[quest].RequiredRaces not in (self.raceIDs['ALLIANCE'], self.raceIDs['HORDE'])) and npcs.nList[creature].hostileToH:
